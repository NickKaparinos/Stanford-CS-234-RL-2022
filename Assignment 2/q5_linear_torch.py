...
class Linear(DQN):
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        if network == 'q_network':
            out = self.q_network(state.reshape(state.shape[0], -1))
        else:
            out = self.target_network(state.reshape(state.shape[0], -1))
    ...
    def calc_loss(self, q_values: Tensor, target_q_values: Tensor,
        ...
        Q_samp = rewards.clone() + gamma * (~done_mask) * torch.max(target_q_values, dim=1)[0]
        Q = q_values.gather(dim=1, index=actions.long().reshape(-1, 1))
        loss = torch.nn.functional.mse_loss(Q_samp, Q.reshape(-1, ))
        return loss
